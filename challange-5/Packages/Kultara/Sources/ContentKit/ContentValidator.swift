...
public enum ContentValidator {
    ...
    public static func validate(
        ...
        if let regionMap = bundle.manifest.regionMap {
            ...
            // A declared pyramid with no manifest in it is worse than no pyramid: the surfaces
            // would fall back to the single PNG and the reader would see the resolution they were
            // told had been fixed. The `{z}/{x}/{y}.png` tree is not enumerated here — it is
            // generated by `scripts/build-map-tiles.sh` and describes itself in `tiles.json`.
            if let tiles = regionMap.tiles, !assets.exists("\(tiles)/tiles.json") {
                findings.append(ValidationFinding(
                    rule: .v14, path: "manifest.json",
                    message: "Region map tile pyramid \"\(tiles)\" has no tiles.json."))
            }
