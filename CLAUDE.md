...
**The app's bundle id is `com.astungkara.hisplora`**, to match the Apple Sign in Services ID. It was `com.umar.hisplora` originally; every `simctl launch`/`simctl install` example anywhere in this file's history used that old id. Commit `80e0c9c` (2026-08-25) reverted it to `com.umar.hisplora` in both `project.pbxproj` and `supabase/config.toml`, which broke Sign in with Apple on TestFlight — the identity token's `aud` is the bundle id, and nothing Apple or GoTrue holds answered to it. Restored 2026-08-27. **The three must agree: `PRODUCT_BUNDLE_IDENTIFIER`, the registered Apple App ID / Services ID, and `[auth.external.apple] client_id` in `config.toml` — and the last one only takes effect after `supabase config push`.**
...
- **The shipped content is one authored quest over five real places, and it is partly unverified.**
  ...
  Pemecutan → Pura Maospahit → Pasar Badung → Catur Muka → Museum Bali. **The quest id no longer
  ...
  architecture, Pasar Badung's address, four-storey layout, 1977 founding and market rules, and
